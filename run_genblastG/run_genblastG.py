...
def clean_gff(file_gff):
    ...
    #将gene_id改为chr:start-end的形式，并对cluster以染色体顺序排序,返回cluster_list
    for cluster in cluster_list:
        for line_list in cluster:
            #print(line_list)
            if line_list[2] == 'mRNA':
                id = str(f'{line_list[0]}:{line_list[3]}-{line_list[4]}')
                line_list[8] = f'ID={id};Name={id}'
...
for cluster in clean_gff('./gene_gff'):
    for line_list in cluster:
